...
// src/pages/contrast/RightEyeContrastScreen.tsx
...
type Props = NativeStackScreenProps<RootStackParamList, "RightEyeContrast">;
...
export default function RightEyeContrastScreen({ navigation }: Props) {
...
//   const [level, setLevel] = useState<number | null>(null);
  ...
  const handleSubmit = async () => {
    ...
    if (sortedUser === sortedCorrect) {
      ...
      setScore(prev => {
        const newScore = Math.round((prev + pts) * 100) / 100;
        return newScore;
      });
    //   setLevel(QUESTION_POINTS[index]);
    }
  ...
  const finishQuiz = async () => {
    // use current score (already accumulated)
    ...
    // Save to backend (if you have saveTestScore util)
    try {
      const userId = await AsyncStorage.getItem("user_id");
      if (userId) {
        await saveTestScore({
          userId,
          testName: "Contrast Sensitivity - Right Eye",
          testTotalScore: 18,
          testScore: score,
          remark,
        });
  ...
  return (
    <Provider>
      <ScrollView contentContainerStyle={styles.container}>
        <Text style={styles.title}>Right Eye Contrast Sensitivity Test</Text>
        ...
        <TextInput
          label="Enter the letters visible to you"
          value={input}
